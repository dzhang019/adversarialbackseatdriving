...
def find_user_content_end_index(bundle: QwenVLBundle, prompt: str, input_ids: torch.Tensor) -> int:
    return find_user_content_span(bundle, prompt, input_ids)[1]


def find_user_content_span(bundle: QwenVLBundle, prompt: str, input_ids: torch.Tensor) -> tuple[int, int]:
    return find_user_content_start_index(bundle, prompt, input_ids), find_user_content_append_index(bundle, prompt, input_ids)


def find_user_content_start_index(bundle: QwenVLBundle, prompt: str, input_ids: torch.Tensor) -> int:
    sentinel = "<LARGO_STORY_CONTENT_BOUNDARY_4d0f2f1e>"
    extended = bundle.processor.apply_chat_template(
        [_user_message(sentinel + prompt)],
        tokenize=True,
        add_generation_prompt=False,
        return_dict=True,
        return_tensors="pt",
    )
    extended = _move_batch_to_device(extended, bundle.device)["input_ids"][0]
...
def find_user_content_append_index(bundle: QwenVLBundle, prompt: str, input_ids: torch.Tensor) -> int:
    sentinel = "<LARGO_STORY_CONTENT_BOUNDARY_4d0f2f1e>"
    extended = bundle.processor.apply_chat_template(
        [_user_message(prompt + sentinel)],
        tokenize=True,
        add_generation_prompt=False,
        return_dict=True,
        return_tensors="pt",
